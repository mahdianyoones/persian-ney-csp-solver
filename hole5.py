...
class H5(HOLES):
	...
	def __init__(self, csp):
		self.__csp = csp		
		self.__h = csp.get_spec("h5")
		self.__space = csp.get_spec["hmarg"] * 3
		self.__space += csp.get_spec["holed"] * 2
		self.__impact_map = {
			"L1": {"L2", "L3", "L4"},
			"L2": {"L1", "L3", "L4"},
			"L3": {"L1", "L2", "L4"},
	...
	def b_update(self, reduced_vars):
		'''Establishes h5 indirect consistency.'''
	...
	def __establish(A, ims):
		'''Implements both direct and indirect consistency for h5.'''	
		if len(ims) == 0:
		...
		for var, new_upper in new_uppers.items():
			if new_upper < D[var]["min"]:
				return (CONTRADICTION, self.__confset(ims), "h5")
	...
	def establish(self, curvar, value):
		'''Establishes h5 direct consistency.'''
		A = self.__csp.get_assignment()
