...
class HOLE6():
	'''Establishes L1 + L2 + L3 + L4 + L5 + S < h6.
	
	S = hole_margin * 1
	
	This constraint guarantees that the 6th hole falls somewhere AFTER the 
	junction of nodes 5 & 6.
	
	From the relation, the following inequalities can be derived:
	
	1) L1_max < h6 - L2_min - L3_min - L4_min - L5_min - S
	2) L2_max < h6 - L1_min - L3_min - L4_min - L5_min - S
	3) L3_max < h6 - L1_min - L2_min - L4_min - L5_min - S
	4) L4_max < h6 - L1_min - L2_min - L3_min - L5_min - S
	5) L5_max < h6 - L1_min - L2_min - L3_min - L4_min - S
	...
	consistent:
 	...
 	from 1,	L1_max = h6 - L2_min - L3_min - L4_min - L5_min - S - 1
	from 2,	L2_max = h6 - L1_min - L3_min - L4_min - L5_min - S - 1
	from 3,	L3_max = h6 - L1_min - L2_min - L4_min - L5_min - S - 1
	from 4,	L4_max = h6 - L1_min - L2_min - L3_min - L5_min - S - 1.
	from 5,	L5_max = h6 - L1_min - L2_min - L3_min - L4_min - S - 1.'''
	...
	def establish(self, csp, curvar, value):
		'''Establishes hole6 domain consistency.
	...
	def __lowers_inconsistent(self, lows, h, s):
		...
		if lows["L1"] >= h-lows["L2"]-lows["L3"]-lows["L4"]-lows["L5"]-s:
			return True
		if lows["L2"] >= h-lows["L1"]-lows["L3"]-lows["L4"]-lows["L5"]-s:
			return True
		if lows["L3"] >= h-lows["L1"]-lows["L2"]-lows["L4"]-lows["L5"]-s:
			return True
		if lows["L4"] >= h-lows["L1"]-lows["L2"]-lows["L3"]-lows["L5"]-s:
			return True
		if lows["L5"] >= h-lows["L1"]-lows["L2"]-lows["L3"]-lows["L4"]-s:
			return True
		return False
