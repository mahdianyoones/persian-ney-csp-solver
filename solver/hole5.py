...
class HOLE5():
	'''Establishes L1 + L2 + L3 + L4 + S < h5.
	
	S = hole_margin * 3 + hole_diameter * 2
	
	This constraint guarantees that the 5th hole falls somewhere AFTER the 
	4th hole and on node 5.
	
	From the relation, the following inequalities can be derived:
	
	1) L1_max < h5 - L2_min - L3_min - L4_min - S
	2) L2_max < h5 - L1_min - L3_min - L4_min - S
	3) L3_max < h5 - L1_min - L2_min - L4_min - S
	4) L4_max < h5 - L1_min - L2_min - L3_min - S
	...
	consistent:
 	...
 	from 1,	L1_max = h5 - L2_min - L3_min - L4_min - S - 1
	from 2,	L2_max = h5 - L1_min - L3_min - L4_min - S - 1
	from 3,	L3_max = h5 - L1_min - L2_min - L4_min - S - 1
	from 4,	L4_max = h5 - L1_min - L2_min - L3_min - S - 1.'''

	def __init__(self, spec):
		self.__h = spec["h5"]
		self.__space = spec["hmarg"] * 3
		self.__space += spec["holed"] * 2
		self.__impact_map = {
			"L1": {"L2", "L3", "L4"},
			"L2": {"L1", "L3", "L4"},
			"L3": {"L1", "L2", "L4"},
	...
	def establish(self, csp, curvar, value):
		'''Establishes hole5 domain consistency.
	...
	def propagate(self, csp, reduced_vars):
		'''Maintains bounds consistency for hole5 due to propagation.'''
		A = csp.get_assignment()
		ims = set([])
